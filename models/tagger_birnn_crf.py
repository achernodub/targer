...
class TaggerBiRNNCRF(TaggerBase):
    ...
    def __init__(self, word_seq_indexer, tag_seq_indexer, class_num, rnn_hidden_dim=100, freeze_word_embeddings=False,
                 dropout_ratio=0.5, rnn_type='GRU', gpu=-1):
        super(TaggerBiRNNCRF, self).__init__(word_seq_indexer, tag_seq_indexer, gpu)
    ...
    def _forward_birnn_cnn(self, word_sequences):
        ...
        features_rnn = self.lin_layer(rnn_output_h) # shape: batch_size x max_seq_len x class_num + 1
        return features_rnn
