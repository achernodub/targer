...
class TaggerBiRNN(TaggerBase):
    ...
    """
    def __init__(self, sequences_indexer, class_num, rnn_hidden_dim=100, freeze_embeddings=False, dropout_ratio=0.5,
                 rnn_type='GRU', gpu=-1):
        super(TaggerBiRNN, self).__init__(sequences_indexer, gpu)
        ...
        if rnn_type == 'GRU':
            self.birnn_layer = LayerBiGRU(input_dim=self.embeddings.embedding_dim, hidden_dim=rnn_hidden_dim, gpu=gpu)
        elif rnn_type == 'LSTM':
            self.birnn_layer = LayerBiLSTM(input_dim=self.embeddings.embedding_dim, hidden_dim=rnn_hidden_dim, gpu=gpu)
        else:
    ...
    def forward(self, inputs_tensor): # inputs_tensor shape: batch_size x max_seq_len
