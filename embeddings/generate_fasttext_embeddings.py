...
if __name__ == "__main__":
    ...
    parser.add_argument('--fn_fasttext_emb_bin', default='wiki.en.bin', help='Fasttext binary model.')
    ...
    for n, word in enumerate(datasets_bank.unique_words_list):
        ...
        emb_vector_str = np.array2string(emb_vector, max_line_width=1000,edgeitems=False).replace('[', '').replace(']', '')
        out_file.write('%s %s\n' % (word, emb_vector_str))
        if n % 100 == 0:
